...
public interface DefaultBinaryTree<T> extends Iterable<T> {
    ...
    interface Visitor2<T> {

        void visit(TreeNode<T> node, T value, int level);
    }
    ...
    interface TreeNode<T> extends Iterable<T> {
        ...
        default void preOrderVisit(Visitor2<T> visitor) {
            // данный класс нужен только для того, чтобы "спрятать" его метод (c 3-мя параметрами)
            class Inner {
                void preOrderVisit(TreeNode<T> node, Visitor2<T> visitor, int level) {
                    if (node == null) {
                        return;
                    }
                    visitor.visit(node, node.getValue(), level);
